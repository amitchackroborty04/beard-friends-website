  ...
  const slides = [
    {
      title: "Beard Friends",
      description:
        "The barber online community offered by Beard Friends is a great way for individuals to connect and share their passion for proper beard maintenance. The community allows users to find nearby barber shops, participate in beard contests, and shop for grooming items, all while connecting with other like-minded individuals.",
    },
    {
      title: "Join Our Community",
      description:
        "Connect with fellow beard enthusiasts from around the world. Share tips, tricks, and experiences about beard care and maintenance. Participate in exciting contests, discover the best barber shops in your area, and access exclusive grooming products.",
    },
    {
      title: "Expert Grooming Tips",
      description:
        "Learn from the best in the business. Our platform brings together professional barbers, grooming experts, and experienced beard enthusiasts to share their knowledge. Get personalized advice, discover new products, and master the art of beard maintenance.",
    },
  ];

  return (
    <div className="relative md:min-h-screen bg-black overflow-hidden">
      {/* Background Image */}
      <div
        className="absolute inset-0 bg-cover bg-center bg-no-repeat"
        style={{
      ...
      >
        {/* Optimized Overlay for better text contrast on mobile */}
        <div className="absolute inset-0 bg-black/70 sm:bg-black/80"></div>
      </div>

      {/* Content Container */}
      <div className="relative z-10 flex flex-col items-center justify-center md:min-h-screen px-6  py-20">
        <div className="text-center w-full max-w-5xl mx-auto">
              ...
              {slides.map((slide, index) => (
                <CarouselItem key={index}>
                  <div className="flex flex-col items-center space-y-4 md:space-y-8">
                    {/* Responsive Title */}
                    <h1 className="text-4xl sm:text-5xl md:text-7xl font-bold text-white tracking-tight px-2">
                      {slide.title}
                    </h1>

                    {/* Responsive Description - Improved line height and max-width for mobile */}
                    <p className="text-gray-300 text-sm sm:text-base md:text-lg max-w-prose mx-auto leading-relaxed md:!leading-[186%] opacity-90">
                      {slide.description}
                    </p>
                    
                    {/* Optional Mobile-Friendly CTA (Common in Hero sections) */}
                    <button className="mt-4 px-8 py-3 bg-purple-600 text-white rounded-full font-semibold md:hidden active:scale-95 transition-transform">
                        Explore Now
                    </button>
                  </div>
                </CarouselItem>
          ...
          {/* Pagination Dots - Larger hit area for thumbs */}
            ...
            {slides.map((_, index) => (
              <button
                ...
                className={`h-2.5 transition-all duration-300 cursor-pointer ${
                  current === index
                    ? "bg-purple-500 w-10 md:w-12 rounded-full"
                    : "bg-white/30 w-2.5 rounded-full hover:bg-white/50"
                }`}
